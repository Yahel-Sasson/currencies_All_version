...
async function init(){
  try{
     showSpinner();
     const coins:Coin[] = await  service.getAllCoins();
     ...
     if( searchField?.value === "All" ){
      ...
      $('.searchBtn').on("click",  function(event){
        ...
        if(searchField?.value === "All" || searchField?.value === "all" || searchField?.value === undefined || searchField?.value === "" ){
          console.log(searchField?.value);
...
$(function() {
  ...
  $('.btnSaveChanges').on("click",  function(event){
   ...
   $('#container').on("click",'.boxToggle' , function(event) {
    // Find the checkbox element inside the clicked boxToggle 
    console.log("cilked me")
    ...
    if (isChecked) {
      ...
      // when the checkbox is  turn to not checked
...
function showAllCardsOfCoin(arrOfTheCoins:Coin[]){
  arrOfTheCoins.forEach(coin => {
    const cardBig = generateCardOfCoin(coin);
    cardRowContainer?.appendChild(cardBig);
  });
}

function generateCardOfCoin(coin:Coin) {
       ...
       <div class="card" style="width: 250px;" id="${coin.id}">
        <div class="card-body" id="${coin.name}">
         ...
         <div class="boxToggle" class="btn btn-primary">
           <div class="form-check form-switch" id= "toggle_${coin.name}">
...
function getExactMatchFilterCurrency(arrOfCoins:Coin[], idOfCoin:string){
  ...
  const exactMatches:Coin[] = arrOfCoins.filter(coin => coin.symbol.toLocaleLowerCase() === idOfCoin.toLocaleLowerCase());
  ...
  <div class="card" style="width: 250px;" id="${exactMatches[0].id}">
   <div class="card-body">
    ...
    <div class="boxToggle"    class="btn btn-primary">
      <div class="form-check form-switch" id= "toggle_${exactMatches[0].id}">
      <input class="form-check-input text-end" type="checkbox" >
      </div>
    </div>
